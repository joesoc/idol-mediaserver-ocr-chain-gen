...
./tools/gen_chain.py --list

# 2. Verify all definitions are correctly deployed
./tools/gen_chain.py --audit

# 3. Preview what would be generated (no files written)
./tools/gen_chain.py tools/definitions/wa_dl.yaml --dry-run

# 4. Generate for real
./tools/gen_chain.py tools/definitions/wa_dl.yaml

# 5. Restart MediaServer, process a sample image, inspect the debug PNG
#    output/debug/*_wa_dl_licence_number_pre_ocr.png
#
# 6. If the box is wrong, edit the lua/compute_dynamic_region_wa_dl_licence_number.lua
...
#### `lua_script`  *(optional, per-field)*

If a field has custom or hand-written Lua that cannot be auto-generated by the
standard `card_estimation` template, set `lua_script` to the filename of that
script (relative to `lua/`).  The tool will skip generating a compute script
for that field and reference the named file in the engine stanza instead.

```yaml
fields:
  - name: Number
    lua_script: compute_dynamic_region_qld.lua   # hand-written; two-branch feature detection
    roi: { left: 0.65, top: 0.03, width: 0.33, height: 0.11 }
    ocr: { ... }
```

All states added before `gen_chain.py` existed (ACT, NSW, QLD, TAS, VIC) use
this override.  Their YAML definitions document the geometry but the actual Lua
logic lives in the hand-written scripts.

> **Naming note:** Generated scripts follow the pattern
> `compute_dynamic_region_<id_lower>_<field_lower>.lua`.  Hand-written scripts
> use the shorter `compute_dynamic_region_<state_lower>.lua` form.
> The `lua_script` override bridges this gap.
...
```
usage: gen_chain.py [-h] [--list] [--audit] [--dry-run] [--force]
                    [--config-dir PATH] [--defs-dir PATH] [definition]
...
options:
  ...
  --audit             Cross-check all definitions against deployed artifacts
  ...
  --defs-dir PATH     Override the definitions/ directory used by --audit
```
...
Run `--audit` to verify that every YAML definition is fully and correctly wired
into all deployed artifacts.  For each definition it checks:

| Artifact | Checks |
|---|---|
| Lua scripts | `filter_<id>.lua`, compute Lua (name from `lua_script` override or default), `draw_<id>.lua` |
| `Kapish.cfg` | Engine appears in `[Session]`; result wired in `[CombineOCRResults]` |
| `Kapish_debug.cfg` | Same |
| XSL | `<xsl:if>` block for identifier present in `transformed_combined.xsl` |
| Unit test | Field entry present in `_DOC_FIELDS` of `unittestforOCRProdV2.py` |

```bash
./gen_chain.py --audit
```

Expected output (current deployment state):

```
Audit  —  6 definitions in definitions/
========================================================================
  ACT_DL                  ✓ 11/11
  NSW_DL                  ✓ 11/11
  QLD_DL                  ✓ 11/11
  TAS_DL                  ✗  7/11
      ✗  Kapish.cfg [Session]: Filter_TAS_DL missing
      ✗  Kapish.cfg [Session]: DynamicRegion_TAS_DL_Number missing
      ✗  Kapish.cfg [Session]: OCR_TAS_DL_Number missing
      ✗  Kapish.cfg [CombineOCRResults]: OCR_TAS_DL_Number.Result missing
  VIC_DL                  ✓ 11/11
  WA_DL                   ✓ 11/11
========================================================================
  Summary: 74/78 passed,  4 FAILED
```

> **Known gap:** `TAS_DL` is wired in `Kapish_debug.cfg` (passes the unit test)
> but has never been promoted to the production `Kapish.cfg`.  Run
> `./gen_chain.py definitions/tas_dl.yaml` to add it.
...
| Document | Likely identifier | Status |
|---|---|---|
| ACT Driver's Licence | `ACT_DL` | ✓ deployed |
| NSW Driver's Licence | `NSW_DL` | ✓ deployed |
| QLD Driver's Licence | `QLD_DL` | ✓ deployed |
| TAS Driver's Licence | `TAS_DL` | debug only — not in `Kapish.cfg` |
| VIC Driver's Licence | `VIC_DL` | ✓ deployed |
| WA Driver's Licence  | `WA_DL`  | ✓ deployed |
| SA Driver's Licence  | `SA_DL`  | not started |
| NT Driver's Licence  | `NT_DL`  | not started |
| Australian Passport  | `AU_PASSPORT` | not started |
| Medicare Card        | `MEDICARE` | not started — multiple fields |
